...
**Two separate objects**, both wired into the container via `envFrom` (not
individual `env:` entries) because several of the harness's own config keys
contain hyphens (`keycloak-external-url`, `push-reports-to-s3`, ...) —
valid for `envFrom`-sourced keys, but rejected by Kubernetes' stricter
C-identifier validation for `env:` entries' `name` field:

- **`apitestrig.configMap`** → a ConfigMap (non-secret)
- **`apitestrig.secret`** → a Secret (credentials, PII)
...
the eSignet host's certificate into the JVM's own `cacerts` truststore,
following the same pattern as the old Java `apitestrig` chart
